...
typedef edm::RefVector<edm::HepMCProduct, HepMC::GenParticle > GenParticleRefVector2;
...
typedef std::vector<HepMC::GenParticle*>::const_iterator particles_in_const_iterator2;
typedef std::vector<HepMC::GenParticle*>::const_iterator particles_out_const_iterator2;
...
struct TrackEvent{
  ...
  int   mmotherid[MAXTRACKS];
  float mmotherPt[MAXTRACKS];
  float mmotherEta[MAXTRACKS];
  float mmotherPhi[MAXTRACKS];
  int   mmothernofdau[MAXTRACKS];
  int   mmotherStatus[MAXTRACKS];
  int   mmothermotherid[MAXTRACKS];
  float mmothermotherPt[MAXTRACKS];
  float mmothermotherEta[MAXTRACKS];
  float mmothermotherPhi[MAXTRACKS];
  int   mmothermothernofdau[MAXTRACKS];
  int   mmothermotherStatus[MAXTRACKS];
...

{
  ...
  doSimVertex_             = iConfig.getUntrackedParameter<bool>  ("doSimVertex",true);
...
//   edm::Handle< GenEventInfoProduct > GenInfoHandle;
  ...
  edm::Handle<GenParticleCollection> higenParticles;

//  iEvent.getByLabel("hiGenParticles",higenParticles);
  iEvent.getByLabel("hiGenParticles",genParticles);
...
//  cout << "number of genparticle" << genParticles->size() << endl;
//  cout << "number of higenparticle" << higenParticles->size() << endl;
//  cout << "number of trackingparticle" <<TPCollectionHeff->size() << endl;
...
//	cout << "trackingparticle pt =" << tp->pt() << " pid =" << tp->pdgId() << " eta=" << tp->eta() << " status=" << tp->status()<< endl;
...
//fill mother information
       if (!tp->genParticle().empty()) {
         const HepMC::GenParticle * genMom = getGpMother2(tp->genParticle()[0].get());
         if (genMom) {
     //    cout << "mother pz=" << genMom->momentum().pz() << " pt =" << genMom->momentum().perp() << " eta = " << genMom->momentum().eta() << " phi = " << genMom->momentum().phi()<< " id =" << genMom->pdg_id() << " n of dau =" << genMom->end_vertex()->particles_out_size()  << endl;
        
		   pev_.mmotherid[pev_.nParticle] = genMom->pdg_id();
	       pev_.mmotherPt[pev_.nParticle] = genMom->momentum().perp();
		   pev_.mmotherEta[pev_.nParticle] = genMom->momentum().eta();
		   pev_.mmotherPhi[pev_.nParticle] = genMom->momentum().phi();
		   pev_.mmotherStatus[pev_.nParticle] = genMom->status();
		   pev_.mmothernofdau[pev_.nParticle] = genMom->end_vertex()->particles_out_size();

		   const HepMC::GenParticle * genMomMom = getGpMother2(genMom);

		   if ( genMomMom ) 
		   {
             pev_.mmothermotherid[pev_.nParticle] = genMomMom->pdg_id();
             pev_.mmothermotherPt[pev_.nParticle] = genMomMom->momentum().perp();
             pev_.mmothermotherEta[pev_.nParticle] = genMomMom->momentum().eta();
             pev_.mmothermotherPhi[pev_.nParticle] = genMomMom->momentum().phi();
             pev_.mmothermotherStatus[pev_.nParticle] = genMomMom->status();
             pev_.mmothermothernofdau[pev_.nParticle] = genMomMom->end_vertex()->particles_out_size();
		   }
		 }
      }
...
//
    ...
    ++pev_.nParticle;
//	cout << "num of particles = " << pev_.nParticle << endl;
  }
...
TrackAnalyzer::beginJob()
{
  ...
  if (doSimTrack_) {
	...
	trackTree_->Branch("mmotherid",&pev_.mmotherid,"mmotherid[nParticle]/I");
	trackTree_->Branch("mmotherPt",&pev_.mmotherPt,"mmotherPt[nParticle]/F");
	trackTree_->Branch("mmotherEta",&pev_.mmotherEta,"mmotherEta[nParticle]/F");
	trackTree_->Branch("mmotherPhi",&pev_.mmotherPhi,"mmotherPhi[nParticle]/F");
	trackTree_->Branch("mmothernofdau",&pev_.mmothernofdau,"mmothernofdau[nParticle]/I");
	trackTree_->Branch("mmotherStatus",&pev_.mmotherStatus,"mmotherStatus[nParticle]/I");

    trackTree_->Branch("mmothermotherid",&pev_.mmothermotherid,"mmothermotherid[nParticle]/I");
    trackTree_->Branch("mmothermotherPt",&pev_.mmothermotherPt,"mmothermotherPt[nParticle]/F");
    trackTree_->Branch("mmothermotherEta",&pev_.mmothermotherEta,"mmothermotherEta[nParticle]/F");
    trackTree_->Branch("mmothermotherPhi",&pev_.mmothermotherPhi,"mmothermotherPhi[nParticle]/F");
    trackTree_->Branch("mmothermothernofdau",&pev_.mmothermothernofdau,"mmothermothernofdau[nParticle]/I");
    trackTree_->Branch("mmothermotherStatus",&pev_.mmothermotherStatus,"mmothermotherStatus[nParticle]/I");
